...
function RedisStore(options) {
  ...
  if (options.client) {
    ...
    pw = options.pw || options.password;
    db = options.db || options.database;
    if (typeof db !== 'number') {
      if (typeof db === 'string') db = parseInt(db, 10);
    }
    if (db && typeof db !== 'number') db = parseInt(db, 10);
    if (!(typeof db === 'number' && !isNaN(db))) db = null;
    if (pw) {
      client.auth(pw, function() {
        if (typeof db === 'number' && !isNaN(db)) client.select(db);
      });
...
RedisStore.prototype.set = function() {
  ...
  if (err && typeof cb === 'function') cb(err, 0);
  return this;
};
...
RedisStore.prototype.del = function() {
  ...
  if (key) {
    ...
    if (typeof cb === 'function') cb(new Error('Invalid key'));
  }
  return this;
};
